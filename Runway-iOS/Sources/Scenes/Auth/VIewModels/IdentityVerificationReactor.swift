...
final class IdentityVerificationReactor: Reactor, Stepper {
    ...
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .viewDidLoad:
            ...
            steps.accept(AppStep.alert(alertTitle, description, [option1, option2]) { action in
                switch action.title {
                ...
                case option2:
                    ...
                    let action = "안녕, 또 만나요!"
                    steps.accept(AppStep.alert(title, desc, action, { _ in steps.accept(AppStep.back)}))
                default:
                    break
                }
            })
